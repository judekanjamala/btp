...
from init_zpd import initial_zpd
...
def main():
    ...
    init_zpd_student = Student(student1_kc_states,
    ...
    zpdes_student_with_init = Student(student1_kc_states,
                            transition_probabilities_zpdes,
                            slip_probabilities,
                            guess_probabilities,
                            get_prerequisites,
                            get_kc,
                            get_solution)
    zpdes_student_without_init = Student(student1_kc_states,
                            transition_probabilities_zpdes,
                            slip_probabilities,
                            guess_probabilities,
                            get_prerequisites,
    ...
    init_zpd = initial_zpd(progression_graph,
                            trace_problems_dict,
                            init_zpd_regularisation_0,
                            init_zpd_entropy_threshold,
                            init_zpd_student)

    # logger.info(f"Before init_zpd {init_zpd_student.status()}")
    # logger.info(f"After init_zpd {init_zpd_student.status()}")
    # # for trace, problems in trace_problem_dict.items():
    # #     for problem in problems:
    # #         print("Solving {}.".format(problem))
    # #         print(student.solve(problem))
    # #         print(student.status())
    # #         time.sleep(1) 

    zpdes(set(progression_graph[""]),
            trace_problems_dict,
            progression_graph,
            zpdes_beta,
            zpdes_eta,
    ...
    zpdes(init_zpd,
            ...
            zpdes_gamma,
            zpdes_initial_weight,
            zpdes_student_with_init)
   


    logger.info(f"{zpdes_student_with_init.status()}")
    logger.info(f"{zpdes_student_without_init.status()}")
